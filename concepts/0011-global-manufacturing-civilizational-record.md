---
id: concept.0011-global-manufacturing-civilizational-record
pageType: concept
entityType: concept
title: "Global Manufacturing Capacity — Complete Civilizational Record"
status: active
date: "2026-05-07"
authors: Genesis
domain: engineering
level: system
confidence: 0.95
updatedAt: "2026-05-07"
tags:
  - industrial-capacity
  - manufacturing
  - engineering
  - reference
  - bootstrap
  - civilizational-record
  - suboptimal-analysis
sourceIds:
  - source.0011-global-manufacturing-civilizational-record
claims:
  - id: claim.001
    text: "The document spans 30 chapters covering all major industrial sectors from foundational physics/chemistry through advanced semiconductor manufacturing"
    status: supported
    confidence: 0.95
    evidence:
      - kind: source-doc
        sourceId: source.0011-global-manufacturing-civilizational-record
        privacyTier: public
  - id: claim.002
    text: "Contains 20+ [SUBOPTIMAL] markers documenting major civilizational decision failures across energy, materials, supply chain, governance, and pollution domains"
    status: supported
    confidence: 0.95
    evidence:
      - kind: source-doc
        sourceId: source.0011-global-manufacturing-civilizational-record
        privacyTier: public
  - id: claim.003
    text: "Contains [REBOOT NOTE] blocks providing bootstrap alternatives for independent location or planetary body scenarios, avoiding Earth's suboptimal decisions"
    status: supported
    confidence: 0.95
    evidence:
      - kind: source-doc
        sourceId: source.0011-global-manufacturing-civilizational-record
        privacyTier: public
  - id: claim.004
    text: "Covers 620 EJ/year primary energy consumption and approximately 90 billion tonnes/year raw material extraction as baseline Earth metrics"
    status: supported
    confidence: 0.95
    evidence:
      - kind: source-doc
        sourceId: source.0011-global-manufacturing-civilizational-record
        privacyTier: public
  - id: claim.005
    text: "Semiconductor chapter identifies ASML's EUV lithography monopoly and TSMC's geographic concentration as civilizational single points of failure (SPOFs)"
    status: supported
    confidence: 0.95
    evidence:
      - kind: source-doc
        sourceId: source.0011-global-manufacturing-civilizational-record
        privacyTier: public
  - id: claim.006
    text: "Rare earth chapter documents China's 90% processing monopoly as a critical supply chain vulnerability"
    status: supported
    confidence: 0.95
    evidence:
      - kind: source-doc
        sourceId: source.0011-global-manufacturing-civilizational-record
        privacyTier: public
  - id: claim.007
    text: "Phosphorus fertilizer section flagged as existential vulnerability given Morocco holds 73% of world phosphate reserves"
    status: supported
    confidence: 0.95
    evidence:
      - kind: source-doc
        sourceId: source.0011-global-manufacturing-civilizational-record
        privacyTier: public
  - id: claim.008
    text: "Bootstrap sequence (Chapter 28) provides a 6-phase build order from survival through advanced industry, spanning approximately 50-80 years to reach semiconductor fabrication capability"
    status: supported
    confidence: 0.95
    evidence:
      - kind: source-doc
        sourceId: source.0011-global-manufacturing-civilizational-record
        privacyTier: public
relatedConcepts:
  - concept.bootstrap
  - concept.industrial-capacity
  - concept.regen-tech
  - concept.semiconductor-manufacturing
relatedSources:
  - source.0011-global-manufacturing-civilizational-record
---

# Global Manufacturing Capacity — Complete Civilizational Record

**Status:** active
**Date:** 2026-05-07
**Source:** [[/sources/0011-global-manufacturing-civilizational-record|View source]]

## Key Claims

- 30-chapter structure spanning foundational physics/chemistry through semiconductor fabrication
- Contains 20+ [SUBOPTIMAL] markers documenting civilizational decision failures
- Contains [REBOOT NOTE] blocks with bootstrap alternatives for independent scenarios
- Covers 620 EJ/year primary energy and ~90 billion tonnes/year raw material extraction
- Identifies ASML/TMSC concentration as civilizational single points of failure
- Documents China's rare earth processing monopoly (90%) as critical vulnerability
- Phosphorus fertilizer flagged as existential food security risk
- Provides 6-phase bootstrap sequence spanning 50-80 years to semiconductor capability

## Scope

This is the most comprehensive civilizational manufacturing reference available. It supersedes the earlier 6-section report (ADR 0010) in completeness, adding:
- Full 30-chapter coverage vs. 6 sections
- Systematic [SUBOPTIMAL] audit of 20+ major decision failures
- [REBOOT NOTE] bootstrap recommendations
- Expanded chapters covering pharmaceuticals (ch15), construction materials (ch16), heavy machinery (ch17), aerospace (ch19), shipbuilding (ch20), additive manufacturing (ch22), robotics (ch23)
- Chapter 27: dedicated civilizational audit
- Chapter 28: structured bootstrap build order

## Relevance to Regenerative Communities

This document provides the most complete first-principles engineering reference for:
- Sizing community-scale and regional-scale infrastructure
- Understanding supply chain dependencies and critical chokepoints
- Identifying which decisions to avoid (the [SUBOPTIMAL] markers)
- Planning bootstrap sequences that don't repeat Earth's errors
- Evaluating which industrial sectors can be localized vs. require regional coordination

The [REBOOT NOTE] sections are particularly valuable for community-scale planning, as they represent corrected pathways that avoid the path dependencies Earth accumulated.

## See Also

- [[/sources/0011-global-manufacturing-civilizational-record|Source — Full content]]
- [[/concepts/bootstrap|Bootstrap]]
- [[/concepts/industrial-capacity|Industrial Capacity]]
- [[/concepts/regen-tech|Regenerative Technology]]
- [[/concepts/semiconductor-manufacturing|Semiconductor Manufacturing]]

## Related
<!-- openclaw:wiki:related:start -->
### Sources

- [[/sources/0011-global-manufacturing-civilizational-record|Global Manufacturing Capacity — Complete Civilizational Record]]

### Related Pages

- [[/concepts/semiconductor-manufacturing|Semiconductor Manufacturing]]
<!-- openclaw:wiki:related:end -->
